    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None, new_xyz=None, scores=None):
        ...
        if new_xyz is None:
            ...
            for i in range(len(self.sample_method_list)):
                ...
                src_scores_slice = scores[:, range_start: range_end].contiguous() \
                ...
                if 'sec:' in self.sample_method_list[i]:
                    part = fps_type.split('-')
                    sec_num = int(part[0][4:])
                    fps_type = '-'.join(part[1:])

                    ry = torch.atan2(src_xyz_slice[..., 1], src_xyz_slice[..., 0])  # (B, N)
                    _, indices_slice = ry.sort(dim=-1, descending=False)
                    xyz_slice = torch.gather(src_xyz_slice, 1, indices_slice.unsqueeze(-1).repeat(1, 1, src_xyz_slice.shape[-1])).view(batch_size * sec_num, -1, src_xyz_slice.shape[-1])
                    scores_slice = torch.gather(src_scores_slice, 1, indices_slice).view(batch_size * sec_num, -1) \
                        if src_scores_slice is not None else None
                    features_slice = torch.gather(src_features_slice, 1, indices_slice.unsqueeze(-1).repeat(1, 1, src_features_slice.shape[-1])).view(batch_size * sec_num, -1, src_features_slice.shape[-1]) \
                        if src_features_slice is not None else None
                    indices_slice = indices_slice.view(batch_size * sec_num, -1)
                else:
                    sec_num = 1
                    xyz_slice = src_xyz_slice
                    scores_slice = src_scores_slice
                    features_slice = src_features_slice
                    indices_slice = None

                assert self.npoint_list[i] % sec_num == 0
                npoints = self.npoint_list[i] // sec_num
                ...
                if fps_type == 'd-fps':
                    ...
                    sample_idx = pointnet2_utils.furthest_point_sample_weights(xyz_slice, scores_slice, npoints)
                else:
                    raise NotImplementedError
        ...
        if self.confidence_mlp is not None:
            ...
            new_scores = new_scores.squeeze(1).contiguous()  # (B, npoint)
...
class PointnetSAModuleFS(PointnetSAModuleFSMSG):
    ...
    def __init__(self, *,
        ...
        super().__init__(
            ...
            weight_gamma=weight_gamma, aggregation_mlp=aggregation_mlp, confidence_mlp=confidence_mlp
        )
