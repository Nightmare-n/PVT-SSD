...
def create_nuscenes_info(dataset_cfg, version, data_path, save_path, max_sweeps=10):
    from nuscenes.nuscenes import NuScenes
    from nuscenes.utils import splits
    from . import nuscenes_utils

    client = getattr(file_client, dataset_cfg.BACKEND.NAME)(
        **dataset_cfg.BACKEND.get('KWARGS', {})
    )
    ...
    available_scenes = nuscenes_utils.get_available_scenes(nusc, client)
    ...
    if version == 'v1.0-test':
        ...
        client.dump_pickle(train_nusc_infos, save_path / f'nuscenes_infos_{max_sweeps}sweeps_test.pkl')
    else:
        print('train sample: %d, val sample: %d' % (len(train_nusc_infos), len(val_nusc_infos)))
        client.dump_pickle(train_nusc_infos, save_path / f'nuscenes_infos_{max_sweeps}sweeps_train.pkl')
        client.dump_pickle(val_nusc_infos, save_path / f'nuscenes_infos_{max_sweeps}sweeps_val.pkl')
